...
- 每页只保留一个 `#` 一级标题；
- 从 `##` 开始组织主要章节，不跳跃标题层级；
- 使用 `##` 表示主要主题，主题下的具体任务或相关能力使用 `###`；存在明确父子关系时不要把所有标题平铺成同一级，也不要为了增加层级而创建只有一个子标题的空泛分组；
- 标题应具体表达任务或能力，避免“其他”“更多”等模糊词；
- 使用简体中文和一致的产品术语；
- 首次出现专业术语时给出中文解释，必要时附英文缩写；
- 将按键上方以橙色标出的功能统一称为“Shift 功能”，只在首次帮助用户定位时说明橙色标签；
- 面向用户描述功能、结果和操作，不解释编程语言、计算引擎、解析管道或前后端实现；开源鸣谢可以保留项目名称，但能力说明仍使用用户能够理解的语言；
- 不记录编写过程、争论或临时决策；
...
- 指示用户点击、按下、短按或长按具有明确文字或符号的计算器按键时，统一使用 `<kbd className="key-cap">按键标签</kbd>`；不要用反引号代替键帽；
- 顶栏、侧边栏、垃圾桶等没有文字标签的图标按钮使用自然语言描述，不虚构键帽文字；功能名称、模式状态和普通术语不因出现在同一段中而改成键帽；
